...
func main() {
    ...
    // exitIfErr(err)
    product := models.Product{Name: "peanuts", Description: "Honey Roasted peanuts", MetaDescription: "Good taste food"}


	// Index a doc using Structs
	_, err := c.Index("productindex", "product", "producId_1", nil, product)
	exitIfErr(err)
	...
	out, err := c.Search("productindex", "product", nil, searchJson)
